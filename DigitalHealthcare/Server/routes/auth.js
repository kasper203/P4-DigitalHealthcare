...
router.post("/register", async (req, res) => {
  try {
    ...
    try {
      ...
      const encryptedMultifaSecret = encryptField(multifa_secret);

      await connection.execute(
        `INSERT INTO Login (user_id, username, password, type, multifa_secret)
         VALUES (?, ?, ?, ?, ?)`,
        [newUserId, cleanUsername, hashedPassword, "patient", encryptedMultifaSecret]
      );
...
router.post("/login", loginLimiter, async (req, res) => {
  try {
    ...
    if (!otpRegex.test(String(otp_code).trim())) {
    ...
    try {
      ...
      console.error('Failed to decrypt multifa_secret:', err);
    }
